...
call "%WINDOWS_ENV%"
"%VENV_PYTHON%" "%PROJECT_DIR%scripts\windows_postgres.py" --check
...
pushd "%BACKEND_DIR%"
"%VENV_PYTHON%" "%PROJECT_DIR%scripts\windows_postgres.py" uvicorn app.main:app --host 0.0.0.0 --port %BACKEND_PORT% --no-access-log
