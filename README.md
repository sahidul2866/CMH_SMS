...
- **Settings → Dropdown options** manages patient sources (OPD, IPD/ward, referral and custom options), military designations, priorities and room choices. VIP must be selected explicitly in patient priority; other dropdowns never select it automatically. Honorifics such as Mr./Mrs. are not part of reception registration.
