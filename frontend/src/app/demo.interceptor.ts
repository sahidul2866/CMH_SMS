...
function dashboard() {
  ...
  return { scope: 'all', generated_at: new Date().toISOString(), total: tokens.length, waiting: tokens.filter((item) => item.status === 'waiting').length, called: tokens.filter((item) => ['called', 'recalled'].includes(item.status)).length, in_progress: tokens.filter((item) => item.status === 'in_progress').length, completed: tokens.filter((item) => item.status === 'completed').length, average_wait_minutes: 6, vip_total: vip.length, vip_waiting: vip.filter((item) => item.status === 'waiting').length, vip_active: vip.filter((item) => ['called', 'recalled', 'in_progress'].includes(item.status)).length, vip_completed: vip.filter((item) => item.status === 'completed').length, radiographers: rows, rooms: rows };
}
